import { TKeys, TPosition, TPositionAndSize } from './types';
import { createImage } from './helpers';
import config from './config';

import playerImg from './assets/player/1.png';

export type TCharButtons = {
...
type TCharConstructor = {
  canvas: HTMLCanvasElement;
  ctx: CanvasRenderingContext2D;
  position: TPosition;
  buttons: TCharButtons;
};
...
type TCharMoviment = 'left' | 'right' | 'jump' | 'attack' | 'fall' | 'stop';
...
export class Char {
  ...
  constructor({ canvas, ctx, position, buttons }: TCharConstructor) {
    this.canvas = canvas;
    this.ctx = ctx;
    ...
    this.image = createImage(playerImg);
  ...
  move(direction: TCharMoviment): void {
    ...
    switch (direction) {
      ...
      case 'left':
        ...
        if (this.position.x >= 20) this.position.x -= this.velocity.x;
  ...
  public handleKeyEvent(key: string, isPressed: boolean) {
    switch (key) {
      case 'a':
        this.keys.moveToLeft.pressed = isPressed;
        this.buttons.left.classList[isPressed ? 'add' : 'remove']('press');
        break;
      case 'd':
        this.keys.moveToRight.pressed = isPressed;
        this.buttons.right.classList[isPressed ? 'add' : 'remove']('press');
        break;
      case 'j':
        this.keys.jump.pressed = isPressed;
        this.buttons.jump.classList[isPressed ? 'add' : 'remove']('press');
        break;
      case 'k':
        this.keys.attack.pressed = isPressed;
        this.buttons.attack.classList[isPressed ? 'add' : 'remove']('press');
        break;
    }
  }

  private createKeyDownEvent({ key }: KeyboardEvent) {
    this.handleKeyEvent(key, true);
  }

  private createKeyUpEvent({ key }: KeyboardEvent) {
  ...
  createKeyEvents() {
    document.addEventListener('keydown', this.createKeyDownEvent.bind(this));
    document.addEventListener('keyup', this.createKeyUpEvent.bind(this));

    this.createButtonEvent({ button: this.buttons.left, key: 'a' });
    this.createButtonEvent({ button: this.buttons.right, key: 'd' });
    this.createButtonEvent({ button: this.buttons.jump, key: 'j' });
    this.createButtonEvent({ button: this.buttons.attack, key: 'k' });

    this.removeButtonEvent({ button: this.buttons.left, key: 'a' });
    this.removeButtonEvent({ button: this.buttons.right, key: 'd' });
    this.removeButtonEvent({ button: this.buttons.jump, key: 'j' });
    this.removeButtonEvent({ button: this.buttons.attack, key: 'k' });
  }

  removeKeyEvents() {
    document.removeEventListener('keydown', this.createKeyDownEvent.bind(this));
    document.removeEventListener('keyup', this.createKeyUpEvent.bind(this));
  }

  draw(): void {
    this.ctx.fillStyle = '#900';
    this.ctx.fillRect(this.canvas.width - 220, 20, 200, 25);
    ...
    this.ctx.fillText(`Mortes: ${this.deathCount}`, this.canvas.width - 20, 70);

    this.ctx.save();
    if (this.flipImage) this.ctx.scale(-1, 1);
    this.ctx.drawImage(
      this.image,
  ...
  update({ animationId }: { animationId: number }): void {
    this.setHitBox();
    this.setAttackHitBox();

    // fall into the hole
    if (this.hitBox.y > this.canvas.height) {
      this.isJumping = true;
      this.position.y = 0;
      this.position.x = 10;
      this.velocity.y = 0;
    ...
    // Moviments
    if (this.keys.attack.pressed) this.move('attack');
    else if (this.keys.moveToRight.pressed && !this.isAttacking)
      this.move('right');
    else if (this.keys.moveToLeft.pressed && !this.isAttacking)
      this.move('left');
    else {
      if (!this.isJumping && !this.isAttacking) this.move('stop');
    }

    if (this.keys.jump.pressed && !this.isJumping) this.move('jump');
